...
// 1. QUICKSORT — Particionamiento de 3 vías (Dutch National Flag)
// Soluciona el problema de O(n²) con duplicados:
// divide en tres zonas: < pivot | == pivot | > pivot
// Los elementos iguales al pivote quedan en su posición final
// y no participan en las llamadas recursivas.
void quickSort3Way(vector<string>& arr, int low, int high) {
    if (low >= high) return;
 
    // Elegir pivote como elemento central (reduce el peor caso en datos casi ordenados)
    int mid = low + (high - low) / 2;
    if (arr[mid] < arr[low])  swap(arr[mid], arr[low]);
    if (arr[high] < arr[low]) swap(arr[high], arr[low]);
    if (arr[mid] < arr[high]) swap(arr[mid], arr[high]);
    // Ahora arr[high] es la mediana de los tres — mejor pivote
 
    string pivot = arr[high];
 
    // lt: límite izquierdo de la zona "== pivot"
    // gt: límite derecho de la zona "== pivot"
    // i: elemento que se está examinando
    ...
    while (i <= gt) {
        if (arr[i] < pivot) {
            ...
            swap(arr[i], arr[gt--]);
            // No incrementar i: el elemento traído desde gt aún no fue examinado
        } else {
            i++;
        }
    }
    // Resultado: arr[low..lt-1] < pivot
    //            arr[lt..gt]    == pivot  (ya en posición final)
    //            arr[gt+1..high] > pivot
...
// FUNCIÓN PRINCIPAL
int main() {
    // Descomentar la siguiente línea para regenerar dataset.txt desde words_alpha.txt:
    ...
    quickSort3Way(dataQuick, 0, (int)dataQuick.size() - 1);
    ...
    cout << "Estructura Base (std::vector para Quick/Heap): ~"
         << (vectorMem + stringsMem) / 1024 << " KB" << endl;
 
    size_t avlNodeSize  = sizeof(AVLNode);
    size_t avlTotalMem  = originalData.size() * avlNodeSize;
    cout << "Estructura Adicional (Nodos de Arbol AVL): ~"
         << avlTotalMem / 1024 << " KB adicionales" << endl;
